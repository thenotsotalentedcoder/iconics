...
function UploadCard({ label, description, accept, fieldName, currentUrl, isImage, onUpload, uploading }) {
    ...
    >
      ...
      {!isImage && currentUrl && !hasNewFile && (
        <div className="mb-4 flex items-center gap-3 px-4 py-3 rounded-xl" style={{ background: 'rgba(62,139,135,0.06)', border: '1px solid rgba(62,139,135,0.12)' }}>
          ...
          <span className="text-sm font-medium" style={{ color: DARK }}>PDF uploaded</span>
        </div>
      )}
      ...
      >
        ...
        <span className="text-sm font-medium" style={{ color: hasNewFile ? TEAL : MUTED }}>
          {hasNewFile ? fileName : `Click to select ${isImage ? 'image' : 'PDF'}`}
        </span>
        <input
          ref={inputRef}
          type="file"
...
export default function AdminCallForPapers() {
  ...
  const handleUpload = async (fieldName, file) => {
    ...
    try {
      ...
      setSettings(r.data);
      const labels = { poster: 'Poster', authorGuidelines: 'Author Guidelines', reviewerGuidelines: 'Reviewer Guidelines' };
    ...
    <div className="p-6 lg:p-8 max-w-4xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="mb-8">
        ...
        <p className="text-sm mt-1" style={{ color: MUTED }}>
          Upload the CFP poster image and guideline PDFs shown on the public Call for Papers page.
        </p>
      </motion.div>
      ...
      {loading ? (
        ...
        <div className="grid gap-6">
          ...
          <UploadCard
            ...
            isImage={false}
            onUpload={handleUpload}
            uploading={uploading === 'authorGuidelines'}
          />
          ...
          <UploadCard
            ...
            isImage={false}
            onUpload={handleUpload}
            uploading={uploading === 'reviewerGuidelines'}
          />
        </div>
      )}
    </div>
